...
## 2024-07-02 - [Prevent Local Privilege Escalation via PYTHONPATH]
**Vulnerability:** The Dockerfile appended `:$PYTHONPATH` to the `PYTHONPATH` environment variable without checking if it was initialized. If `PYTHONPATH` is uninitialized, this results in a trailing colon (e.g., `/myData/myTools:`), which Python interprets as adding the current working directory (an empty string) to `sys.path`. This introduces a path hijacking or local privilege escalation vulnerability where a malicious script in the current directory could be executed instead of a standard library module.
**Learning:** Appending potentially uninitialized variables in path-like environment variables can silently introduce insecure defaults, such as including the current working directory in the search path.
**Prevention:** Always use bash conditional variable substitution (e.g., `${PYTHONPATH:+:${PYTHONPATH}}`) to only append the colon and the variable if the variable is actually set and non-empty.
